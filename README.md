...
- view
  - components
    - atoms
      - widgetの最小パーツ
      - appBarをAtomsに入れると型の関係でStatelesswidgetを継承できない。よって他のAtomと書き方が異なるので、appBarをAtomsに入れるか検討中。
    - molecules
      - テキストやボタンを組み合わせた最小構成
    - organisms
...
- const
  - 定数の定義  
    - enum
      - １つの２つ以上の定数を宣言できるものに関してはEnumからデータを引っ張ってくるようにしています。
        - 例えば、githubAPIから取得できるリポジトリのスター数には、「スター数」という定数とスターアイコンが存在するためEnumを使用して紐付けを行っています。
